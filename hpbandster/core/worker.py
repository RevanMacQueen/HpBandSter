...
class Worker(object):
    ...
    def __init__(self,  run_id, log_path=None, nameserver=None, nameserver_port=None, logger=None, host=None, id=None, timeout=None):
        """
        ...
        self.worker_id = "hpbandster.run_%s.worker.%s.%i" % (
            self.run_id, socket.gethostname(), os.getpid())
    ...
    def load_nameserver_credentials(self, working_directory, num_tries=60, interval=1):
        ...
        """
        fn = os.path.join(working_directory,
                          'HPB_run_%s_pyro.pkl' % self.run_id)
        ...
        for i in range(num_tries):
            try:
                ...
                self.logger.warning(
                    'config file %s not found (trail %i/%i)' % (fn, i+1, num_tries))
                time.sleep(interval)
            except:
                raise
        raise RuntimeError(
    ...
    def _run(self):
        ...
        try:
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                self.logger.debug(
                    'WORKER: Connected to nameserver %s' % (str(ns)))
                dispatchers = ns.list(
                    prefix="hpbandster.run_%s.dispatcher" % self.run_id)
        except Pyro4.errors.NamingError:
            if self.thread is None:
                raise RuntimeError('No nameserver found. Make sure the nameserver is running at that the host (%s) and port (%s) are correct' % (
                    self.nameserver, self.nameserver_port))
            else:
                self.logger.error('No nameserver found. Make sure the nameserver is running at that the host (%s) and port (%s) are correct' % (
                    self.nameserver, self.nameserver_port))
                exit(1)
        except:
            raise
        ...
        for dn, uri in dispatchers.items():
            try:
                ...
                self.logger.debug(
                    'WORKER: Dispatcher did not respond. Waiting for one to initiate contact.')
                pass
            except:
                raise
    ...
    def start_computation(self, callback, id, *args, **kwargs):
        ...
        try:
            ...
            self.logger.debug(
                'WORKER: done with job %s, trying to register it.' % str(id))
            with self.thread_cond:
                self.busy = False
                callback.register_result(id, result)
                self.thread_cond.notify()
        self.logger.info(
            'WORKER: registered result for job %s with dispatcher' % str(id))
