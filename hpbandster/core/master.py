...
class Master(object):
    def __init__(self,
        ...
        if job_queue_sizes[0] >= job_queue_sizes[1]:
            raise ValueError(
                "The queue size range needs to be (min, max) with min<max!")

        if previous_result is None:
            self.warmstart_iteration = []

        else:
            self.warmstart_iteration = [WarmStartIteration(
                previous_result, self.config_generator)]
        ...
        self.dispatcher = Dispatcher(self.job_callback, queue_callback=self.adjust_queue_size, run_id=run_id,
                                     ping_interval=ping_interval, nameserver=nameserver, nameserver_port=nameserver_port, host=host)
    ...
    def get_next_iteration(self, iteration, iteration_kwargs):
        ...
        raise NotImplementedError(
            'implement get_next_iteration for %s' % (type(self).__name__))
    ...
    def run(self, n_iterations=1, min_n_workers=1, iteration_kwargs={},):
        ...
        if self.time_ref is None:
            ...
            self.logger.info('HBMASTER: starting run at %s' %
                             (str(self.time_ref)))
        ...
        while True:
            ...
            if not next_run is None:
                self.logger.debug(
                    'HBMASTER: schedule new run for iteration %i' % i)
                self._submit_job(*next_run)
                continue
            else:
                if n_iterations > 0:  # we might be able to start the next iteration
                    self.iterations.append(self.get_next_iteration(
                        len(self.iterations), iteration_kwargs))
                    n_iterations -= 1
                    continue
    ...
    def adjust_queue_size(self, number_of_workers=None):
        ...
        self.logger.debug(
            'HBMASTER: number of workers changed to %s' % str(number_of_workers))
        with self.thread_cond:
            ...
            if self.dynamic_queue_size:
                ...
                self.job_queue_sizes = (
                    self.user_job_queue_sizes[0] + nw, self.user_job_queue_sizes[1] + nw)
                self.logger.info(
                    'HBMASTER: adjusted queue size to %s' % str(self.job_queue_sizes))
    ...
    def job_callback(self, job):
        ...
        with self.thread_cond:
            self.logger.debug(
                'job_callback for %s got condition' % str(job.id))
    ...
    def _submit_job(self, config_id, config, budget):
        ...
        """
        self.logger.debug(
            'HBMASTER: trying submitting job %s to dispatcher' % str(config_id))
        with self.thread_cond:
            self.logger.debug(
                'HBMASTER: submitting job %s to dispatcher' % str(config_id))
            self.dispatcher.submit_job(
                config_id, config=config, budget=budget, working_directory=self.working_directory)
            self.num_running_jobs += 1

        # shouldn't the next line be executed while holding the condition?
        self.logger.debug(
            "HBMASTER: job %s submitted to dispatcher" % str(config_id))
    ...
    def active_iterations(self):
        ...
        l = list(filter(lambda idx: not self.iterations[idx].is_finished, range(
            len(self.iterations))))
        return(l)
